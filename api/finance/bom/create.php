...
$required = ['bom_number', 'product_id', 'version', 'status', 'components', 'total_cost'];
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id FROM bom WHERE bom_number = :bom_number");
    $stmt->execute([':bom_number' => $_POST['bom_number']]);
    if ($stmt->fetch()) {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO bom (
            bom_number, product_id, version, status, description,
            overhead_percentage, total_cost, notes, created_by
        ) VALUES (
            :bom_number, :product_id, :version, :status, :description,
            :overhead_percentage, :total_cost, :notes, :created_by
        )
    ...
    $logStmt->execute([
        ...
        ':action' => 'create_bom',
        ':details' => "Created BOM {$_POST['bom_number']} for product with " . count($components) . " components"
    ]);
